...
export async function splitAudioFile(filePath, splits, outputDir) {
  ...
  for (let i = 0; i < splits.length; i++) {
    ...
    await new Promise((resolve, reject) => {
      ...
      filters.push('silenceremove=start_periods=1:start_threshold=-40dB:start_duration=0.05:stop_periods=-1:stop_threshold=-50dB:stop_duration=0.5');
...
export async function processAlbumSide(filePath, outputDir, expectedInfo = null) {
  ...
  const trackGapBuffer = 0.5; // seconds to trim from end to avoid next track's intro
  for (let i = 0; i < splits.length; i++) {
